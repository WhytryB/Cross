...
def home(request):
    ...
    # (1,"Buy house")
    name_of_goal = wte_m.Goal.objects.filter(owner=request.user).values_list ('id', 'name')

    # <Making a query string.
    query = []
    for wallet in name_of_goal:
        q = ('SELECT "goal_total" AS "goal" FROM "goal" WHERE "goal".id =') + str(int(wallet[0]))
        query.append (q)
    ...
    balance_of_every_goal = []
    if query:
        query += ';'
        # [{'balance': None}, {'balance': Decimal('-4.00')}, {'balance': Decimal('3.00')}]
        balance_of_every_goal = sql.sql(query=query)
        for key, totals in enumerate(balance_of_every_goal):
            wallet_title = name_of_goal[key][1]
            totals.update({'goals': wallet_title})
    ...
    #     # [{'balance': None}, {'balance': Decimal('-4.00')}, {'balance': Decimal('3.00')}]
    #     for key, totals in enumerate(balance_of_every_goal):
    #         print(wallets_of_user,"----------------------")
    #         print(balance_of_every_goal,"00000000000000000")
    #         print(key)
    #         d = list(totals)
    #         print(d,"22222222222222222222222222222222")
    #         wallet_title = wallets_of_user[d[0]][1]
    #
    #         totals.update({'wallet': wallet_title})
    ...
    if query:
        ...
        for key, totals in enumerate (balance_of_every_goal):
            s = totals.get("wallet")
            try:
                cel = int(totals.get("goal")) - int(totals_by_wallet[0].get("sum_of_income"))
                if cel <= 0:
                    wallet_title = "success, you have {0}".format(abs(cel))
                else:
                    wallet_title = cel
                totals.update({'raznica': wallet_title})
            except IndexError:
                totals.update({'raznica': "No transaction"})
    ...
    return render(request, 'dashboard/dashboard.html', {
        ...
        'goal_total': balance_of_every_goal or None,
        'dashboard': True,
    })
