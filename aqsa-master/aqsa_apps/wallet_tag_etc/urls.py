...
from . import views_goal as goal
...
wallet_tag_etc_urlpatterns = [
    ...
    url(r'^goal/list$', goal.List.as_view(), name='goal_list'),
    url(r'^goal/new$', goal.Create.as_view(), name='goal_new'),
    url(r'^goal/edit/(?P<pk>\d+)$', goal.Update.as_view(), name='goal_edit'),
    url(r'^goal/delete/(?P<pk>\d+)$', goal.Delete.as_view(), name='goal_delete'),
