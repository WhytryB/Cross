...
class Goal(models.Model):
    ...
    owner = models.ForeignKey(User, models.CASCADE)
    name = models.CharField(max_length=50, validators=[v.two_or_more_spaces], verbose_name=_('Name'))
    goal_total = models.PositiveIntegerField(verbose_name=_('Goal'))
    wallet12 = models.ForeignKey(Wallet, models.PROTECT, verbose_name=_('Wallet12'))

    def __str__(self):
        return self.name

    def get_update_url(self):
        return reverse_lazy('wallet_tag_etc:goal_edit', kwargs={'pk': self.pk})

    def get_delete_url(self):
        return reverse_lazy('wallet_tag_etc:goal_delete', kwargs={'pk': self.pk})

    links = {
        'create': (reverse_lazy('wallet_tag_etc:goal_new'), _('New Goal'), 'fas fa-file-signature'),
        'list': (reverse_lazy('wallet_tag_etc:goal_list'), _('List of Goal'), 'fas fa-list-ul'),
    }