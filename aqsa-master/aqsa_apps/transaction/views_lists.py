...
from aqsa_apps.transaction import models as t_m
...
class ListFilter(List):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['data_filter'] = self.date
        context['value_filter'] = self.value

        context['titles_of_labels'] = dict((label, m.Transaction._meta.get_field(label).verbose_name) for label in
                                           ('transfer', 'wallet', 'category', 'tag', 'currency', 'contact','date','value',))
        ...
        context['data'] = t_m.Transaction.objects.filter(owner = self.request.user)
        context['value'] = t_m.Transaction.objects.filter (owner=self.request.user)
